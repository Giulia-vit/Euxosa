...
function transformPropGetters(path, transform) {
  ...
  if (types.isFunctionDeclaration(node.declaration)) {
    ...
    node.declaration = types.variableDeclaration('const', [types.variableDeclarator(functionId, transform(functionDeclarationToExpression(node.declaration)))]);
    return;
  }

  if (types.isVariableDeclaration(node.declaration)) {
    node.declaration.declarations.forEach(function (declaration) {
      if (types.isIdentifier(declaration.id) && functionsToReplace.includes(declaration.id.name) && declaration.init) {
...
function addWithFixNodeFileTraceImport(path) {
  return helperModuleImports.addNamed(path, HOFName, importFrom);
}
  ...
  return {
    ...
    visitor: {
      Program: function Program(path, state) {
        ...
        var filePath = (_getFileName = getFileName(state)) != null ? _getFileName : nodePath.join('pages', 'Default.js');

        if (isPage(filePath)) {
          var body = path.get('body');
          body.filter(function (path) {
            return types.isExportNamedDeclaration(path);
          }).forEach(function (path) {
            transformPropGetters(path, function (decl) {
              return types.callExpression(addWithFixNodeFileTraceImport(path), [decl]);
            });
          });
          return;
          ...
          var exportDefaultDeclaration = _body.find(function (path) {
            return types.isExportDefaultDeclaration(path);
          });
